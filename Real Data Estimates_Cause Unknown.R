...
S1;z1 # recall with cause 1
S2;z2 # recall with cause 2
S3    # non-recall
S4    # right-censored 
...
aa=60 # Scaling of data
z1=z1/aa;S1=S1/aa;z2=z2/aa;S2=S2/aa;S3=S3/aa;S4=S4/aa  
...
for(j in 1:it){
  ...
  i11=sapply(1:length(S3),function(ii){integrate(function(u){i1(S3[ii],u)},lower1,S3[ii],subdivisions=100L,stop.on.error=FALSE)$value});i11
  i2=function(S,u){m.th4*m.th5*(u^(m.th4-1))*exp(-m.th2*(u^m.th1)-m.th5*(u^m.th4))*(1-exp(-(S-u)*m.th6))}
  i12=sapply(1:length(S3),function(ii){integrate(function(u){i2(S3[ii],u)},lower1,S3[ii],subdivisions=100L,stop.on.error=FALSE)$value});i12
  p_i<-i11/(i11+i12);p_i
  P<-sum(p_i);P
  
  #Expected Values
  dd1=exp(-m.th2*(lower1^m.th1))-exp(-m.th2*(S3^m.th1));dd1  # Denominator 1
  dd2=exp(-m.th5*(lower1^m.th4))-exp(-m.th5*(S3^m.th4));dd2  # Denominator 2
  
  int01=function(u){m.th1*m.th2*(u^m.th1)*exp(-m.th2*(u^m.th1))}
  intt01=array(0,length(S3))
  ...
  dd3=exp(-m.th3*lower1)-exp(-m.th3*(S3-xi01));dd3 
  ee3=exp(-m.th3*lower1)-exp(-m.th3*(S3-xi02));ee3
  
  SS1=S3-xi01;SS1
  int6=function(u){u*m.th3*exp(-m.th3*u)}
  intt6=array(0,length(S3))
  for(i in 1:length(S3)){
    l=lower1
    u=SS1[i]
  ...
  m.thh1[j+1]=(n1+P)/(-sum(log(z1))-sum(p_i*xi3)+m.thh2[j]*sum((z1^m.thh1[j])*log(z1))
                      +m.thh2[j]*sum((z2^m.thh1[j])*log(z2))+m.thh2[j]*sum(p_i*xi4)
                      +m.thh2[j]*sum((1-p_i)*xi12)+m.thh2[j]*sum((S4^m.thh1[j])*log(S4)))
...
mn2=aa*ml[5]^(-1/ml[4])*gamma(1+1/ml[4]);mn2 # mean age of operative menopause

# Median duration
md1=aa*ml[2]^(-1/ml[1])*(log(2))^(1/ml[1]);md1 # median age of natural menopause
md2=aa*ml[5]^(-1/ml[4])*(log(2))^(1/ml[4]);md2 # median age of natural menopause
...
#============================================================================#
#Expected Values
ddd1=exp(-ml[2]*(lower1^ml[1]))-exp(-ml[2]*(S3^ml[1]));ddd1  # Denominator 1
ddd2=exp(-ml[5]*(lower1^ml[4]))-exp(-ml[5]*(S3^ml[4]));ddd2  # Denominator 2

int011=function(u){ml[1]*ml[2]*(u^ml[1])*exp(-ml[2]*(u^ml[1]))}
intt011=array(0,length(S3))
for (i in 1:length(S3)){
  l=lower1
  u=S3[i]
...
ddd3=exp(-ml[3]*lower1)-exp(-ml[3]*(S3-xxi01));ddd3 
eee3=exp(-ml[3]*lower1)-exp(-ml[3]*(S3-xxi02));eee3

SS11=S3-xxi01;SS11
int66=function(u){u*ml[3]*exp(-ml[3]*u)}
intt66=array(0,length(S3))
for(i in 1:length(S3)){
  l=lower1
  u=SS11[i]
...
se=c(sqrt(diag(Var_Cov)[1]),sqrt(diag(Var_Cov)[2]),sqrt(diag(Var_Cov)[3]),sqrt(diag(Var_Cov)[4]),sqrt(diag(Var_Cov)[5]),sqrt(diag(Var_Cov)[6]));round(se,4)
...
it1=250000
for(j in 1:it1){
  j1=function(S,u){b.th1*b.th2*(u^(b.th1-1))*exp(-b.th2*(u^b.th1)-b.th5*(u^b.th4))*(1-exp(-(S-u)*b.th3))}
  l11=sapply(1:length(S3),function(ii){integrate(function(u){j1(S3[ii],u)},lower1,S3[ii],subdivisions=100L,stop.on.error=FALSE)$value});l11
  j2=function(S,u){b.th4*b.th5*(u^(b.th4-1))*exp(-b.th2*(u^b.th1)-b.th5*(u^b.th4))*(1-exp(-(S-u)*b.th6))}
  l12=sapply(1:length(S3),function(ii){integrate(function(u){j2(S3[ii],u)},lower1,S3[ii],subdivisions=100L,stop.on.error=FALSE)$value});l12
  q_i=l11/(l11+l12);q_i
  
  z_i=rbinom(length(S3),1,q_i);z_i
  Z=sum(z_i);Z
  
  ww=runif(length(S3));ww
  
  tt1=(-(1/b.th2)*log(exp(-b.th2*(lower1^b.th1))-ww*(exp(-b.th2*(lower1^b.th1))-exp(-b.th2*(S3^b.th1)))))^(1/b.th1);tt1  
  tt2=(-(1/b.th5)*log(exp(-b.th5*(lower1^b.th4))-ww*(exp(-b.th5*(lower1^b.th4))-exp(-b.th5*(S3^b.th4)))))^(1/b.th4);tt2  
  
  u_i=-(1/b.th3)*log(exp(-b.th3*lower1)-ww*(exp(-b.th3*lower1)-exp(-b.th3*(S3-tt1))));u_i 
  v_i=-(1/b.th6)*log(exp(-b.th6*lower1)-ww*(exp(-b.th6*lower1)-exp(-b.th6*(S3-tt2))));v_i
  ...
  b.thh1[j]=b.th1;b.thh2[j]=b.th2;b.thh3[j]=b.th3;b.thh4[j]=b.th4;b.thh5[j]=b.th5;b.thh6[j]=b.th6
...
zz1=seq(90,length(ch1),90);zz2=seq(90,length(ch2),90)
...
ch11=ch111[1:2000];ch22=ch222[1:2000];ch33=ch333[1:2000];ch44=ch444[1:2000];ch55=ch555[1:2000];ch66=ch666[1:2000]
